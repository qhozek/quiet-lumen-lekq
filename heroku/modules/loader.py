...
class LoaderMod(loader.Module):
    ...
    def __init__(self):
        ...
        self.config = loader.ModuleConfig(
            ...
            loader.ConfigValue(
                "autoupdate_modules",
                False,
                lambda: "Auto-reinstall external modules from their URLs on restart (herokusafe: OFF by default, чужой пуш в модуль не подтянется сам)",
                validator=loader.validators.Boolean(),
            ),
        )
    ...
    async def _update_modules(self):
        ...
        elif not self.config.get("autoupdate_modules", False):
            # herokusafe: не перекачиваем чужие модули на каждый рестарт.
            # Раньше любой пуш в modules-репо молча выполнялся после рестарта.
            # Обновление только вручную через .dlmod / при autoupdate_modules=True.
            logger.info(
                "herokusafe: skipping external modules auto-reinstall (%d saved)",
                len(todo),
            )
        else:
